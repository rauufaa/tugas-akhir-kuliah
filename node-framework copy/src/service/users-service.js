...
import { queryBuilder } from "../app/database.js";
...
const getAll = async (page, size) => {
    ...
    query = "SELECT * FROM `User` LIMIT " + skip + ", " + size;
    params = [];
...
const get = async (user_id) => {
    ...
    if (!results) throw new ResponseError(404, "User not found");
...
const post = async (body) => {
    let query = "INSERT INTO `User` (`first_name`, `last_name`, `bio`, `phone_number`, `email`, `gender`) VALUES (?, ?, ?, ?, ?, ?)";
...
const put = async (body, user_id) => {
    let query = "UPDATE `User` SET `first_name`=?, `last_name`=?, `bio`=?, `phone_number`=?, `email`=?, `gender`=? WHERE `user_id`=?";
