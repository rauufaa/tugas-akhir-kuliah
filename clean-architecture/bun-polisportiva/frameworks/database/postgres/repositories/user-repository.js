...
export default function userRepository() {
    ...
    const findByOwnerIdSportNamePaging = (ownerId, sportName, skip, limit) => {
    ...
    const findByOwnerIdPaging = (ownerId, skip, limit) => {
        const results = prisma.sportsField.findMany({
            skip: skip,
            take: limit,
            where: {
    ...
    const findBySportNamePaging = (sportName, skip, limit) => {
        const results = prisma.sportsField.findMany({
            skip: skip,
            take: limit,
            where: {
    ...
    const findByOwnerIdSportName = (ownerId, sportName) => {
        const results = prisma.sportsField.findMany({
            where: {
                owner_id: ownerId,
                sport: sportName
    ...
    const findBySportName = (sportName) => {
        const results = prisma.sportsField.findMany({
            where: {
                sport: sportName
            }
    ...
    const add = (sportsField) => {
        const results = prisma.sportsField.create({
            data: {
                ...
                price_list_id: sportsField.getPriceListId(),
